...
Weebly is a special case worth spelling out: the subscription itself is
cancelled the moment wopha.com cuts over to the new site, which is part of
...
QuickBooks CSV exports, support, and the old Weebly hosting cancelled at the
wopha.com cutover.
...
- Hardware, owned by the association: a PoE license-plate-recognition camera
  ...
  recognition software. One-time hardware cost: $140 for the camera
  (Reolink RLC-811A, an 8MP PoE camera with 5x optical zoom suited to
  entrance plate capture, current Reolink store price) plus $180 for the
  compute node (an N100-class mini PC, 8GB RAM, 256GB storage, enough to run
  Frigate with an open-source plate-recognition model), about $320 total
  plus mounting hardware and cabling, association-owned. This is an estimate
  from current street prices as of July 2026; a final quote will be attached
